  ...
  return (
    <div className="flex-shrink-0 mx-auto sm:mx-0 group relative">
      <div className="w-36 sm:w-40 rounded-2xl overflow-hidden shadow-2xl shadow-black/60 ring-1 ring-white/10">
        <img src={media.cover_url || fallback} alt={media.title}
      ...
      <button onClick={() => setEditing(true)}
        ...
        <span className="text-xs bg-white/20 backdrop-blur-sm border border-white/20 px-3 py-1.5 rounded-lg text-white font-semibold">🖼️ Ganti Cover</span>
      </button>
      ...
      {editing && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/75 backdrop-blur-sm" onClick={() => setEditing(false)}>
          <div className="glass rounded-2xl p-6 w-full max-w-md border border-white/10 animate-in" onClick={e => e.stopPropagation()}>
            ...
            <input type="url" className="input-field mb-2" placeholder="https://contoh.com/gambar.jpg"
      ...
      <div className="glass rounded-2xl p-5 space-y-3 border border-white/5">
        ...
        {collection ? (
          <>
            ...
            {media?.total_episodes && (
              <div>
                <div className="flex justify-between text-xs text-gray-400 mb-1.5">
                  <span>Progress</span>
                  <span className="font-semibold text-white">{collection.progress} / {media.total_episodes} jam</span>
                </div>
                <div className="h-2 bg-white/10 rounded-full overflow-hidden">
                  <div className="h-full bg-gradient-to-r from-primary-500 to-accent-500 rounded-full transition-all" style={{ width: `${Math.min(100, (collection.progress / media.total_episodes) * 100)}%` }} />
                </div>
              </div>
            )}
            {collection.rating && (
            ...
            {collection.review && <p className="text-gray-400 text-xs italic border-t border-white/5 pt-3">"{collection.review}"</p>}
          </>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/75 backdrop-blur-sm" onClick={() => setShowModal(false)}>
          <div className="glass rounded-2xl p-6 w-full max-w-sm border border-white/10 animate-in" onClick={e => e.stopPropagation()}>
            ...
            <div className="space-y-4">
              ...
              {media?.total_episodes && (
                <div>
                  <label className="text-xs text-gray-400 font-semibond uppercase tracking-wide mb-1.5 block">Jam Dimainkan ({form.progress}/{media.total_episodes})</label>
...
// ── ChapterNoteCard & Modal (non-game) ─────────────────────────────────────────
      ...
      <div className="flex">
        {note.cover_url && (
          <div className="flex-shrink-0 w-20 sm:w-24">
            <img src={note.cover_url} alt={`Ch.${note.chapter_number}`} className="w-full h-full object-cover min-h-[100px]"
          ...
          <div className="flex items-start justify-between gap-3">
            <div className="flex items-center gap-3 min-w-0">
              ...
              <div className="min-w-0">
                ...
                <p className="text-gray-500 text-xs">{new Date(note.created_at).toLocaleDateString('id-ID', { day: 'numeric', month: 'short', year: 'numeric' })}</p>
              </div>
            </div>
...
function ChapterNoteModal({ mediaId, editNote, onClose, onSaved }) {
  ...
  const handleSubmit = async () => {
    if (!form.chapter_number || !form.note.trim()) { setError('Nomor chapter dan catatan wajib diisi!'); return; }
    setLoading(true); setError('');
    try {
      if (editNote) await api.patch(`/chapters/${editNote.id}`, { chapter_title: form.chapter_title, note: form.note, mood: form.mood, cover_url: form.cover_url || null });
      else await api.post('/chapters', { media_id: mediaId, chapter_number: parseInt(form.chapter_number), chapter_title: form.chapter_title || null, note: form.note, mood: form.mood, cover_url: form.cover_url || null });
      onSaved(); onClose();
      ...
      <div className="glass rounded-2xl p-6 w-full max-w-md border border-white/10 animate-in max-h-[92vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <input type="url" placeholder="URL gambar chapter (opsional)" className="input-field" value={form.cover_url} onChange={set('cover_url')} />
            {form.cover_url && <div className="mt-2 w-16 rounded-lg overflow-hidden ring-1 ring-white/10"><img src={form.cover_url} alt="preview" className="w-full aspect-[3/4] object-cover" onError={e => { e.target.parentElement.style.display = 'none'; }} /></div>}
          </div>
          <div>
            ...
            <div className="grid grid-cols-5 gap-1.5">
              {MOODS.map(m => (
                <button key={m.key} type="button" onClick={() => setForm(f => ({ ...f, mood: m.key }))}
                  className={`flex flex-col items-center gap-1 p-2 rounded-xl border transition-all ${form.mood === m.key ? 'bg-primary-600/30 border-primary-500/60 scale-105' : 'bg-white/5 border-white/5 hover:bg-white/10'}`}>
          ...
          <div>
            ...
            <textarea rows={5} placeholder="Tulis tanggapanmu..." className="input-field resize-none text-sm" value={form.note} onChange={set('note')} />
          </div>
        </div>
        <div className="flex gap-2 mt-5">
          ...
          <button onClick={handleSubmit} disabled={loading} className="btn-primary flex-1 py-2.5">
            {loading ? <span className="flex items-center justify-center gap-2"><span className="w-3.5 h-3.5 border-2 border-white/30 border-t-white rounded-full animate-spin" />Menyimpan...</span> : editNote ? 'Simpan' : 'Simpan 📝'}
          </button>
        </div>
      </div>
    </div>
...
function ChapterNotesSection({ mediaId, isLoggedIn }) {
  ...
  const sorted = [...notes].sort((a, b) => sortDesc ? b.chapter_number - a.chapter_number : a.chapter_number - b.chapter_number);
    ...
    <>
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <button onClick={() => setSortDesc(v => !v)} className="text-xs px-3 py-1.5 rounded-lg glass-hover text-gray-400">{sortDesc ? '↓ Terbaru' : '↑ Terlama'}</button>
        ...
        {topMoods.length > 0 && (
          <div className="glass rounded-xl px-4 py-3 flex flex-wrap items-center gap-3 border border-white/5">
            ...
            {topMoods.map(([key, count]) => { const m = getMoodObj(key); return (
              <div key={key} className="flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-white/5 border border-white/10">
                <span className="text-base">{m.emoji}</span><span className="text-xs text-gray-300">{m.label}</span><span className="text-xs text-gray-500">×{count}</span>
              </div>
            ); })}
          </div>
        )}
        {loading ? (
          ...
          <div className="space-y-3">{sorted.map(note => <ChapterNoteCard key={note.id} note={note} onEdit={(n) => { setEditNote(n); setShowModal(true); }} onDelete={handleDelete} />)}</div>
        )}
      </div>
      {showModal && <ChapterNoteModal mediaId={mediaId} editNote={editNote} onClose={() => { setShowModal(false); setEditNote(null); }} onSaved={fetchNotes} />}
    </>
  );
}
      ...
      <div className="glass rounded-2xl p-6 w-full max-w-lg border border-white/10 animate-in max-h-[92vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
        ...
        <div className="space-y-4">
            ...
            <div className="grid grid-cols-4 gap-2">
              {JOURNAL_TYPES.map(t => (
                <button key={t.key} type="button" onClick={() => setForm(f => ({ ...f, entry_type: t.key }))}
                  className={`flex flex-col items-center gap-1 p-2.5 rounded-xl border transition-all ${form.entry_type === t.key ? 'bg-primary-600/30 border-primary-500/60' : 'bg-white/5 border-white/5 hover:bg-white/10'}`}>
          ...
          <div>
            ...
            <input type="text" placeholder="e.g. Cara Ngalahin Malewicz Boss" className="input-field" value={form.title} onChange={set('title')} />
          </div>
          {/* Difficulty + Tags */}
          <div className="grid grid-cols-2 gap-3">
            <div>
            ...
            <div>
              ...
              <input type="text" placeholder="boss, weapon, tip" className="input-field" value={form.tags} onChange={set('tags')} />
            </div>
          </div>
          {/* Cover */}
          <div>
            <label className="text-xs text-gray-400 font-semibold uppercase tracking-wide mb-1.5 block">🖼️ Gambar (opsional)</label>
            <input type="url" placeholder="URL screenshot / gambar" className="input-field" value={form.cover_url} onChange={set('cover_url')} />
            {form.cover_url && <div className="mt-2 h-24 rounded-lg overflow-hidden ring-1 ring-white/10"><img src={form.cover_url} alt="preview" className="w-full h-full object-cover" onError={e => { e.target.parentElement.style.display = 'none'; }} /></div>}
          </div>
          {/* Content */}
          <div>
            <label className="text-xs text-gray-400 font-semibold uppercase tracking-wide mb-1.5 block">Konten / Strategi *</label>
            <textarea rows={6} placeholder="Tulis strategi, tips, atau ceritamu di sini..." className="input-field resize-none text-sm leading-relaxed" value={form.content} onChange={set('content')} />
          </div>
        </div>
        <div className="flex gap-2 mt-5">
          <button onClick={onClose} className="btn-secondary flex-1 py-2.5">Batal</button>
          <button onClick={handleSubmit} disabled={loading} className="btn-primary flex-1 py-2.5">
            {loading ? <span className="flex items-center justify-center gap-2"><span className="w-3.5 h-3.5 border-2 border-white/30 border-t-white rounded-full animate-spin" />Menyimpan...</span> : editEntry ? 'Simpan' : 'Simpan Journal 🗺️'}
          </button>
        </div>
      </div>
    </div>
    ...
    <div className="glass rounded-xl overflow-hidden border border-white/5 hover:border-white/15 transition-all group">
      {entry.cover_url && (
        <div className="h-32 overflow-hidden">
          <img src={entry.cover_url} alt={entry.title} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500"
            onError={e => { e.target.parentElement.style.display = 'none'; }} />
        </div>
      )}
      <div className="p-4">
        <div className="flex items-start justify-between gap-3 mb-2">
          ...
          <div className="flex items-center gap-1.5 flex-shrink-0">
            <span className={`text-xs px-2 py-0.5 rounded-full border font-semibold ${DIFFICULTY_STYLES[entry.difficulty]}`}>{entry.difficulty}</span>
          </div>
        </div>
        {entry.tags && (
          <div className="flex gap-1 flex-wrap mb-2">
            {entry.tags.split(',').map(t => t.trim()).filter(Boolean).map(tag => (
              <span key={tag} className="text-xs px-2 py-0.5 rounded-full bg-primary-500/10 text-primary-400 border border-primary-500/20">#{tag}</span>
            ))}
          </div>
        )}
        <p className={`text-gray-300 text-sm leading-relaxed whitespace-pre-wrap ${!expanded ? 'line-clamp-3' : ''}`}>{entry.content}</p>
        ...
        <div className="flex items-center justify-between mt-3">
          <p className="text-gray-600 text-xs">{new Date(entry.created_at).toLocaleDateString('id-ID', { day: 'numeric', month: 'short', year: 'numeric' })}</p>
      ...
      <div className="glass rounded-2xl p-6 w-full max-w-md border border-white/10 animate-in max-h-[92vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
        ...
        <div className="space-y-4">
            ...
            <div className="grid grid-cols-3 gap-2">
              {ITEM_TYPES.map(t => (
                <button key={t.key} type="button" onClick={() => setForm(f => ({ ...f, item_type: t.key }))}
                  className={`flex items-center gap-2 p-2.5 rounded-xl border transition-all ${form.item_type === t.key ? 'bg-primary-600/30 border-primary-500/60' : 'bg-white/5 border-white/5 hover:bg-white/10'}`}>
          ...
          <div>
            ...
            <input type="text" placeholder="e.g. Oni Genji, Legendary Skin" className="input-field" value={form.name} onChange={set('name')} />
          </div>
          {/* Rarity */}
          <div>
              ...
              {RARITIES.map(r => {
                ...
                return (
                  <button key={r} type="button" onClick={() => setForm(f => ({ ...f, rarity: r }))}
                    className={`px-3 py-1.5 rounded-xl border text-xs font-semibold capitalize transition-all ${form.rarity === r ? `${s.bg} ${s.color} scale-105` : 'bg-white/5 border-white/10 text-gray-400 hover:bg-white/10'}`}>
                    {r}
                  </button>
                );
              })}
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input type="number" min="0" max="100" placeholder="opsional" className="input-field" value={form.power_rating} onChange={set('power_rating')} />
            </div>
            <div>
              <label className="text-xs text-gray-400 font-semibold uppercase tracking-wide mb-1.5 block">Availability</label>
              <input type="text" placeholder="e.g. Event, Shop, Season 3" className="input-field" value={form.availability} onChange={set('availability')} />
            </div>
          </div>
          {/* Image */}
          <div>
            <label className="text-xs text-gray-400 font-semibold uppercase tracking-wide mb-1.5 block">🖼️ Gambar</label>
            <input type="url" placeholder="URL gambar skin/hero" className="input-field" value={form.image_url} onChange={set('image_url')} />
            {form.image_url && <div className="mt-2 w-20 h-20 rounded-xl overflow-hidden ring-1 ring-white/10"><img src={form.image_url} alt="preview" className="w-full h-full object-cover" onError={e => { e.target.parentElement.style.display = 'none'; }} /></div>}
          </div>
          {/* Description */}
          <div>
            <label className="text-xs text-gray-400 font-semibold uppercase tracking-wide mb-1.5 block">Deskripsi</label>
            <textarea rows={2} placeholder="Catatan tentang item ini..." className="input-field resize-none text-sm" value={form.description} onChange={set('description')} />
          </div>
          {/* Owned toggle */}
          <label className="flex items-center gap-3 cursor-pointer">
            <input type="checkbox" checked={form.is_owned} onChange={e => setForm(f => ({ ...f, is_owned: e.target.checked }))} className="w-4 h-4 rounded" />
        ...
        <div className="flex gap-2 mt-5">
          <button onClick={onClose} className="btn-secondary flex-1 py-2.5">Batal</button>
          <button onClick={handleSubmit} disabled={loading} className="btn-primary flex-1 py-2.5">
            {loading ? <span className="flex items-center justify-center gap-2"><span className="w-3.5 h-3.5 border-2 border-white/30 border-t-white rounded-full animate-spin" />Menyimpan...</span> : editItem ? 'Simpan' : 'Tambah Item'}
          </button>
        </div>
      </div>
    </div>
    ...
    <div className={`glass rounded-2xl overflow-hidden border transition-all group hover:-translate-y-1 duration-300 ${rStyle.bg} ${item.rarity !== 'common' ? `shadow-lg ${rStyle.glow}` : ''}`}>
      ...
      <div className="aspect-square relative overflow-hidden bg-white/5">
        ...
        <div className="absolute bottom-2 right-2 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
          <button onClick={() => onEdit(item)} className="w-7 h-7 rounded-lg bg-black/60 hover:bg-white/20 text-white text-xs flex items-center justify-center transition-all">✏️</button>
          <button onClick={() => onDelete(item.id)} className="w-7 h-7 rounded-lg bg-red-500/60 hover:bg-red-500/80 text-white text-xs flex items-center justify-center transition-all">🗑️</button>
        </div>
      </div>
      {/* Info */}
      <div className="p-3">
        ...
        {item.power_rating && (
          <div className="mt-2">
            <div className="h-1 bg-white/10 rounded-full overflow-hidden">
              <div className={`h-full rounded-full ${item.power_rating >= 80 ? 'bg-yellow-400' : item.power_rating >= 50 ? 'bg-blue-400' : 'bg-gray-400'}`}
...
// ── GameSection (kombinasi Journal + Collection) ────────────────────────────────
    ...
    <>
      ...
      <div className="flex gap-2 mb-5">
        {[
          ...
          <button key={t.key} onClick={() => { setActiveTab(t.key); setFilterType('all'); }}
            className={`px-4 py-2 rounded-xl text-sm font-semibold transition-all flex items-center gap-2 ${activeTab === t.key ? 'bg-primary-600 text-white' : 'glass-hover text-gray-400'}`}>
            {t.label}
            <span className={`text-xs px-1.5 py-0.5 rounded-full ${activeTab === t.key ? 'bg-white/20' : 'bg-white/10'}`}>{t.count}</span>
          </button>
        ))}
      </div>
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between flex-wrap gap-2">
            ...
            <div className="flex gap-1 flex-wrap">
              <button onClick={() => setFilterType('all')} className={`text-xs px-3 py-1.5 rounded-xl transition-all ${filterType === 'all' ? 'bg-white/20 text-white' : 'glass-hover text-gray-400'}`}>All</button>
              {JOURNAL_TYPES.map(t => (
                <button key={t.key} onClick={() => setFilterType(t.key)}
                  className={`text-xs px-3 py-1.5 rounded-xl transition-all flex items-center gap-1 ${filterType === t.key ? 'bg-white/20 text-white' : 'glass-hover text-gray-400'}`}>
                  {t.emoji} {t.label}
                </button>
              ))}
            </div>
          ...
          {loadingJ ? (
            ...
            <div className="glass rounded-2xl p-12 text-center border border-white/5">
              ...
              <button onClick={() => { setEditEntry(null); setShowJModal(true); }} className="btn-primary py-2 px-6">Tulis Journal Pertama</button>
            </div>
          ) : (
            <div className="grid sm:grid-cols-2 gap-4">
              {filteredJournal.map(e => <JournalCard key={e.id} entry={e} onEdit={(en) => { setEditEntry(en); setShowJModal(true); }} onDelete={deleteJournal} />)}
            </div>
          )}
        </div>
      )}
      ...
      {activeTab === 'collection' && (
        <div className="space-y-4">
          ...
          {loadingC ? (
            <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-6 gap-3">{[...Array(6)].map((_, i) => <div key={i} className="glass rounded-2xl aspect-square animate-pulse" />)}</div>
            ...
            <div className="glass rounded-2xl p-12 text-center border border-white/5">
              ...
              <button onClick={() => { setEditItem(null); setShowCModal(true); }} className="btn-primary py-2 px-6">Tambah Item Pertama</button>
            </div>
          ) : (
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-3">
              {filteredCollection.map(item => <GameItemCard key={item.id} item={item} onEdit={(i) => { setEditItem(i); setShowCModal(true); }} onDelete={deleteItem} />)}
            </div>
          )}
        </div>
      )}

      {showJModal && <JournalModal mediaId={mediaId} editEntry={editEntry} onClose={() => { setShowJModal(false); setEditEntry(null); }} onSaved={fetchJournal} />}
      {showCModal && <GameItemModal mediaId={mediaId} editItem={editItem} onClose={() => { setShowCModal(false); setEditItem(null); }} onSaved={fetchCollection} />}
    </>
  );
}
    ...
    <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="flex flex-col sm:flex-row gap-6 mb-8">
        ...
        <div className="flex-1 min-w-0 space-y-2.5">
          <div className="flex flex-wrap items-center gap-2">
            ...
            {media.status && <span className={`badge ${media.status === 'completed' ? 'bg-green-500/20 text-green-400 border border-green-500/30' : 'bg-blue-500/20 text-blue-400 border border-blue-500/30'}`}>{media.status}</span>}
          </div>
          ...
          <div className="flex flex-wrap gap-4 text-sm text-gray-400">
            ...
            {media.total_episodes && <span className="flex items-center gap-1.5"><svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 6h16M4 10h16M4 14h16M4 18h16" /></svg>{media.total_episodes} {isGame ? 'jam' : isReadable ? 'chapter' : 'episode'}</span>}
      ...
      <div className="border-b border-white/10 mb-6">
        <div className="flex -mb-px">
          {[{ key: 'info', label: '📄 Info & Sinopsis' }, { key: 'tab2', label: tab2Label }].map(tab => (
